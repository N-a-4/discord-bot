const exampleContainer = new ContainerBuilder()
  ...
  .addActionRowComponents(row => row
    .addComponents(
      new ButtonBuilder().setCustomId("embed:sub-1754934351776-8h96").setStyle(ButtonStyle.Secondary).setEmoji({ id: emojis.back_purp.id }),
...
${emojis.a2} **Причина ухода**: По собственному желанию`))
  ...
  .addSeparatorComponents(separator => separator.setDivider(true).setSpacing(SeparatorSpacingSize.Large))
  .addTextDisplayComponents(textDisplay => textDisplay.setContent(`### Время пребывания на роли`))
  .addTextDisplayComponents(textDisplay => textDisplay.setContent(`${emojis.role_mask_mix} **Фармер**
${emojis.dot_yell} Занимается добычей ресурсов для клана`))
  .addActionRowComponents(row => row.addComponents(new ButtonBuilder().setLabel("71 день").setCustomId("mini:mini-hfypr7z").setStyle(ButtonStyle.Secondary)))
  .addTextDisplayComponents(textDisplay => textDisplay.setContent(`${emojis.role_mask_mix} **Электрик**
${emojis.dot_yell} Обустраивает базу всеми необходимыми электрическими системами`))
  .addActionRowComponents(row => row.addComponents(new ButtonBuilder().setLabel("36 дней").setCustomId("mini:mini-hfypr7z").setStyle(ButtonStyle.Secondary)))
  .addTextDisplayComponents(textDisplay => textDisplay.setContent(`${emojis.role_mask_mix} **Билдер**
${emojis.dot_yell} Отвечает за проектирование, строительство и укрепление базы клана`))
  .addActionRowComponents(row => row.addComponents(new ButtonBuilder().setLabel("4 дня").setCustomId("mini:mini-hfypr7z").setStyle(ButtonStyle.Secondary)))
  .addTextDisplayComponents(textDisplay => textDisplay.setContent(`${emojis.role_mask_mix} **Фермер**
${emojis.dot_yell} Обеспечивает клан стабильным ресурсом еды и трав`))
  .addActionRowComponents(row => row.addComponents(new ButtonBuilder().setLabel("0 дней").setCustomId("btn:btn-1757942268834").setStyle(ButtonStyle.Secondary)))
  .addActionRowComponents(row => row
    .addComponents(
  ...
  .addActionRowComponents(row => row
    .addComponents(
      ...
      new ButtonBuilder().setLabel("Должности").setCustomId("mini:mini-hfypr7z").setStyle(ButtonStyle.Secondary).setEmoji({ id: emojis.position.id })
    )
  )
